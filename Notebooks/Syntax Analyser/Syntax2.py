...
class SyntaxBuilder:
    ...
    def primeros(self, v, parents = set()):
        global max_depth
        max_depth-=1
        ...
        if len(v) == 1 and v[0] in self.non_terminals:
            #print('<<<<<',v[0])
            if v[0] in parents: return self.first[v[0]]
            ...
            for p in productions:
                first |= self.primeros(p, parents | {v[0]} )
            max_depth+=1
            self.first[v[0]] |= first
            return first
        
        first = self.primeros([v[0]], parents)
        
        if self.null_value in first:
            if len(v)>1:
                first.discard(self.null_value)
                first |= self.primeros(v[1:], parents)
        max_depth+=1
        return first
